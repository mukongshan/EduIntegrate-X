...
from .mysql_repository import MySqlCollegeCRepository
from .repository import CourseFullError, InMemoryCollegeCRepository, NotFoundError, ValidationError
...
def create_app(repository=None, gateway=None) -> FastAPI:
    ...
    if repository is None:
        if settings.storage_backend.lower() == "mysql":
            ...
            repository = InMemoryCollegeCRepository(college_id=settings.college_id)
    if gateway is None:
        gateway = MockIntegrationGateway(repository)

    app = FastAPI(title="College C Backend", version="0.1.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
